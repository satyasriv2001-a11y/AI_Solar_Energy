...
# -*- coding: utf-8 -*-
"""
Half-Hourly Predictions for 24 Hours of Test Data

For each 30-minute interval in 24 hours of testing data, predict the next 24 hours' capacity factor.
Each 30-minute interval's prediction is saved as a separate CSV file with plots.
...
Note: This script makes predictions every 30 minutes. If data is hourly, it will use the same hourly data point
for consecutive 30-minute predictions.
"""
...
def run_halfhourly_predictions(data_path, config, output_dir, test_halfhours=48, 
                               credentials_path=None, spreadsheet_name=None, save_plots=True, 
                               plant_name=None, return_predictions=False):
    """
    Run half-hourly predictions for test_halfhours consecutive 30-minute intervals from test data.
    Each 30-minute interval's prediction (next 24 hours) is saved as a separate CSV file with plot.
    
    Args:
        data_path: Path to data CSV file
        config: Model configuration
        output_dir: Directory to save prediction plots
        test_halfhours: Number of consecutive 30-minute intervals from test data to use (default: 48 = 24 hours)
        credentials_path: Path to Google service account JSON file
    ...
    print("=" * 80)
    print("Half-Hourly Predictions for Test Data")
    print("=" * 80)
    print(f"Data file: {data_path}")
    print(f"Model: {config['experiment_name']}")
    print(f"Test half-hours: {test_halfhours} (={test_halfhours*0.5:.1f} hours)")
    ...
    # Check if Minute column exists (30-minute resolution data)
    has_minute_column = 'Minute' in df.columns
    
    if has_minute_column:
        df['Datetime'] = pd.to_datetime(df[['Year', 'Month', 'Day', 'Hour', 'Minute']])
        print("  Detected 30-minute resolution data (Minute column found)")
    else:
    ...
    print("\n[4/4] Making half-hourly predictions...")
    ...
    if len(test_halfhour_indices) < test_halfhours:
        print(f"  Warning: Only found {len(test_halfhour_indices)} valid half-hour intervals (requested {test_halfhours})")
        print(f"  Using available intervals: {len(test_halfhour_indices)}")
    
    print(f"  Making predictions for {len(test_halfhour_indices)} half-hour intervals...")
    ...
    for halfhour_num, halfhour_idx in enumerate(test_halfhour_indices, 1):
        try:
            ...
            # Save to CSV (always save to CSV for 30-minute predictions)
    ...
    print(f"\n{'='*80}")
    print(f"[SUCCESS] Completed predictions for {len(test_halfhour_indices)} half-hour intervals")
    if save_plots:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Make half-hourly predictions for test data. Each 30-minute interval predicts next 24 hours.',
...
Examples:
  ...
  # Use different number of test half-hours (default: 48 = 24 hours)
  python halfhourly_predictions_48h.py --data-path data/Project1140.csv --test-halfhours 48
        """
    )
    ...
    parser.add_argument('--test-halfhours', type=int, default=48,
                       help='Number of consecutive 30-minute intervals from test data to use (default: 48 = 24 hours)')
